/**
 * HJOPF — HJ-OPF benchmark entry point.
 *
 ...
 *   Policy:  Optional staged residual operators after HJ (problem quantities only; no wall-clock control)
 *
...
public final class HJOPF {
    ...
    private static final String OUTPUT_SUMMARY_FILE = System.getProperty("output", "results/runs/HJOPF_summary.csv");
    private static final String LOG_FILE = System.getProperty("log", "results/runs/HJOPF.log");
    ...
    private static final String MODE = System.getProperty("mode", "hash_only");
    private static final boolean COST_RESIDUAL = MODE.equals("adaptive");
    private static final boolean DIAGNOSTIC = Boolean.parseBoolean(System.getProperty("diagnostic", "false"));
    // Bitmap fusion removed from HJ-OPF narrative — default never.
    private static final String BITMAP_POLICY = System.getProperty("bitmapPolicy", "never");
    // Residual policy (optional): HJ → optional CPC → optional Gallop.
    ...
    private static final int BITMAP_MIN_OCCURRENCES = Integer.getInteger("bitmapMinOccurrences",
            COST_RESIDUAL ? 256 : 512);
    private static final double BITMAP_MIN_DENSITY = Double.parseDouble(
            System.getProperty("bitmapMinDensity", COST_RESIDUAL ? "0.20" : "0.25"));
    private static final double BITMAP_MIN_FANOUT = Double.parseDouble(
            System.getProperty("bitmapMinFanout", COST_RESIDUAL ? "1.0" : "1.0"));
    ...
    private static final String WSB_POLICY = System.getProperty("wsbPolicy",
            COST_RESIDUAL ? "cost" : MODE.equals("full") ? "cost" : "never");
    ...
    private static final boolean ADAPTIVE_WSB_CHEAP_PRUNE = Boolean.parseBoolean(
            System.getProperty("adaptiveWsbCheapPrune", COST_RESIDUAL ? "true" : "false"));
    ...
    private static final boolean ADAPTIVE_SMART_INTERSECT = Boolean.parseBoolean(
            System.getProperty("adaptiveSmartIntersect", COST_RESIDUAL ? "true" : "false"));
    /**
    ...
    private static final boolean ADAPTIVE_GALLOP_WITHOUT_CPC = Boolean.parseBoolean(
            System.getProperty("adaptiveGallopWithoutCpc", COST_RESIDUAL ? "true" : "false"));
    /**
    ...
    private static final boolean HASH = MODE.equals("hash_only") || MODE.equals("hash_sparse") ||
            MODE.equals("hash_wsb") || MODE.equals("full") || COST_RESIDUAL;
    ...
    private static final boolean WSB = MODE.equals("wsb_only") || MODE.equals("hash_wsb") ||
            MODE.equals("sparse_wsb") || MODE.equals("full") || COST_RESIDUAL;
    ...
    public static final class FusionEngine {
        ...
        public List<PatternCandidate> generateNextGeneration(List<PatternCandidate> currentGen) {
            ...
            if (COST_RESIDUAL && !ADAPTIVE_WSB_USE_RANGE) {
            ...
            long decisionStart = COST_RESIDUAL ? System.nanoTime() : 0L;
            ...
            boolean costAwareWsb = COST_RESIDUAL && WSB_POLICY.equals("cost");
            ...
            for (PatternCandidate p : currentGen) {
                ...
                for (PatternCandidate q : matchingQs) {
                    ...
                    if (checkWsb) {
                        if (DIAGNOSTIC || COST_RESIDUAL) MetricsTracker.wsbChecksCount++;
                        double supportBound = Math.min(p.prefixSupport * expK, q.suffixSupport);
            ...
            if (COST_RESIDUAL && wsbSampleChecks > 0) {
        ...
        private GenerationDecision chooseGenerationPolicy(List<PatternCandidate> generation,
                                                          Map<IntArrayKey, List<PatternCandidate>> prefixMap) {
            GenerationDecision decision = new GenerationDecision();
            // Bitmap removed from HJ-OPF — always scalar fusion.
            decision.useBitmap = false;
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (BufferedWriter csvWriter = new BufferedWriter(new OutputStreamWriter(
            ...
            String startMsg = "HJOPF mode=" + MODE + " hash=" + HASH + " sparse=" + SPARSE +
                    " wsb=" + WSB + " bitmapPolicy=" + BITMAP_POLICY + " wsbPolicy=" + WSB_POLICY +
                    " diagnostic=" + DIAGNOSTIC +
                    (COST_RESIDUAL ? " bitmapMaxConversionUnits=" + ADAPTIVE_BITMAP_MAX_CONVERSION_UNITS +
            ...
            String endMsg = "HJOPF mode=" + MODE + " finished";
